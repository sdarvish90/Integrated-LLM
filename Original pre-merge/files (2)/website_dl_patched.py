...
def site_irena_publications_items_playwright(
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            for i in range(5):
                page.evaluate("window.scrollTo(0, document.body.scrollHeight)")
                page.wait_for_timeout(1000)            # Extract publication links (robust to JS hydration / headless DOM differences)
            try:
            ...
            # Use Playwright's selector extraction (avoids brittle page.evaluate JS strings)
            ...
            links_data = []
            for h in hrefs:
                if "/publications/" in h.lower():
                    links_data.append({"href": h, "text": ""})

            # De-dup while preserving order
            seen_h = set()
            links_data = [x for x in links_data if not (x["href"] in seen_h or seen_h.add(x["href"]))]
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            for a in anchors:
                ...
                aria = a.get("ariaLabel", "").strip()
                title = a.get("title", "").strip()
                
                if not href:
                    continue
                
                label = text or aria or title or "download"
                href_lower = href.lower()
                label_lower = label.lower()
                ...
                is_download = "download" in label_lower or a.get("download")
                
                # IRENA specific patterns - media files
                is_irena_media = "/-/media/" in href_lower
                
                if is_pdf or is_data or is_download or is_irena_media:
                    full_url = norm_url(page_url, href)
                    
                    # Skip social/external links
                    if any(x in full_url.lower() for x in ["twitter.com", "facebook.com", "linkedin.com"]):
                        continue
    ...
    for it in items:
        ...
        # For Compass: use Playwright to extract downloads (JS-rendered)
            ...
            if not dl_links:
                ...
                try:
                    ...
                    dl_links = []
        # For IRENA: use Playwright to extract downloads
        elif site_type == "irena_publications_playwright":
            dl_links = extract_irena_downloads_playwright(it.page_url)
            if not dl_links:
                # Fallback to generic extraction
                try:
                    html = fetch_html(session, it.page_url, referer=it.page_url)
        ...
        for label, url in dl_links:
            try:
                ...
                is_pdf = ".pdf" in url_lower or "pdf" in label.lower()
