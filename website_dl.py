...
def download_data_file(
    ...
    if h in EXISTING_FILE_HASHES:
        ...
        state["seen_download_urls"].add(url)
...
def download_irena_pdf_via_button(page_url: str, out_dir: str, default_name: str, state: dict) -> Optional[str]:
    ...
    """
    if page_url in state["seen_download_urls"]:
        return None
    ...
    try:
        with sync_playwright() as p:
            ...
            # Step 4: Download using Cmd+S (Mac) or Ctrl+S (Linux/Windows)
            _is_mac = sys.platform == "darwin"
            _save_key = "Meta+s" if _is_mac else "Control+s"
            _save_as_key = "Meta+Shift+s" if _is_mac else "Control+Shift+s"
            _key_label = "Cmd" if _is_mac else "Ctrl"
            print(f"        Pressing {_key_label}+S to download...")

            try:
                with new_page.expect_download(timeout=60000) as download_info:
                    new_page.keyboard.press(_save_key)
                ...
                print(f"        {_key_label}+S failed: {e}, trying {_key_label}+Shift+S...")

                try:
                    with new_page.expect_download(timeout=60000) as download_info:
                        new_page.keyboard.press(_save_as_key)
                    ...
                    print(f"        {_key_label}+Shift+S failed: {e2}, trying click...")
    ...
    if download_path and os.path.exists(download_path):
        ...
        if file_hash in EXISTING_FILE_HASHES or file_hash in state["seen_hashes"]:
            print(f"        [SKIP - DUPLICATE] File with same content already exists")
            os.remove(download_path)  # Delete the just-downloaded duplicate
            state["seen_download_urls"].add(page_url)
            state["seen_hashes"].add(file_hash)
            return None
...
def parse_rss_feed(xml_content: bytes) -> List[dict]:
    """Parse RSS or Atom feed into list of items."""
...
def site_rss_feed_items(
    ...
    try:
        resp = session.get(feed_url, timeout=30, headers={"User-Agent": USER_AGENT})
        resp.raise_for_status()
    except Exception as e:
        print(f"  [ERROR] Failed to fetch RSS feed {feed_url}: {e}")
        return items
    
    feed_items = parse_rss_feed(resp.content)
    
    for fi in feed_items[:max_items]:
        link = fi.get("link", "").strip()
        title = fi.get("title", "").strip()
        
        if not link:
            continue
        
        # Check if title matches keywords
        if keywords and not title_matches(title + " " + fi.get("summary", ""), keywords):
            continue
...
def load_api_keys() -> dict:
    ...
    keys = {"EIA_API_KEY": "", "NREL_API_KEY": ""}
    
    # Try config files
    config_paths = [
        Path("api_keys.json"),
        Path.home() / ".hydrogen_intel" / "api_keys.json"
    ]
...
def site_eia_api_items(
    ...
    api_key = API_KEYS.get("EIA_API_KEY", "")
    if not api_key:
        print(f"  [SKIP] EIA API - No API key. Set EIA_API_KEY in api_keys.json or environment.")
        return items
    
    # For now, treat EIA API endpoint as a page to visit
    # The actual API data integration can be extended later
    print(f"  [INFO] EIA API endpoint configured: {api_endpoint}")
    
    return items


# -------------------------
# Index Page Site Type (for IEA, IRENA, etc.)
# -------------------------
def site_index_page_items(
    session: requests.Session,
    page_url: str,
    site_name: str,
    keywords: List[str],
    ...
    """Crawl an index/publications page and extract relevant links."""
    items = []
    ...
    for a in soup.find_all("a", href=True):
        ...
        # Check if it's a downloadable file or relevant page
        is_download = any(full_url.lower().endswith(ext) for ext in [".pdf", ".xlsx", ".csv", ".zip"])
        
        # Check keywords
        text_to_check = anchor_text + " " + full_url
        if keywords and not title_matches(text_to_check, keywords):
            if not is_download:  # Still include downloads even without keyword match
                continue
        
        items.append(Item(
            title=anchor_text[:200] or "Document",
            page_url=full_url,
            site_name=site_name,
        ))
...
# -------------------------
# FID Signal Site Types
# -------------------------
def site_fid_direct_items(
    start_urls: List[str],
    site_name: str,
    signal_type: str = "fid_signal",
    tier: int = 1,
) -> List[Item]:
    """Direct download URLs for FID signals (e.g., CAISO queue xlsx)."""
    items = []
    for url in start_urls:
        items.append(Item(
            title=f"{site_name} ({signal_type}, Tier {tier})",
            page_url=url,
            site_name=site_name,
        ))
    return items
...
def site_fid_index_items(
    ...
    """Crawl FID signal index pages (queues, permits, awards)."""
    items = []
    ...
    # FID-specific keywords to boost
    fid_keywords = keywords + [
        "queue", "interconnection", "study", "agreement", "permit",
        "award", "grant", "spreadsheet", "download", "xlsx", "csv", "report"
    ]
    ...
    for a in soup.find_all("a", href=True):
        ...
        is_data_file = any(full_url.lower().endswith(ext) for ext in [".xlsx", ".xls", ".csv", ".pdf", ".zip"])
        
        # Check FID keywords
        text_to_check = anchor_text + " " + full_url
        has_fid_keyword = any(kw.lower() in text_to_check.lower() for kw in fid_keywords)
        
        if is_data_file or has_fid_keyword:
            items.append(Item(
                title=f"{anchor_text[:150]} ({signal_type}, Tier {tier})" if anchor_text else f"{site_name} data",
                page_url=full_url,
                site_name=site_name,
            ))
    ...
    # Always include the index page itself if no items found
    if not items:
        items.append(Item(
            title=f"{site_name} ({signal_type}, Tier {tier})",
            page_url=page_url,
...
def run_site(
    ...
    # --- Discovery cache: skip network discovery if all cached items are already processed ---
    discovery_cache = state.setdefault("discovery_cache", {})
    cached = discovery_cache.get(site_name)
    if cached:
        cached_urls = set(cached.get("page_urls", []))
        if cached_urls and cached_urls.issubset(state["seen_pages"]):
            print(f"  All {len(cached_urls)} item(s) from previous discovery already processed. Skipping.")
            return
    ...
    for start_url in start_urls:
        ...
        elif site_type == "rss_feed":
            found_items += site_rss_feed_items(
                session,
                start_url,
                site_name,
                keywords,
        ...
        elif site_type == "index_page":
            found_items += site_index_page_items(
                session,
                start_url,
                site_name,
                keywords,
                allowed_domains=site_cfg.get("allowed_domains", []),
                max_items=int(site_cfg.get("max_items", 100)),
            )

        elif site_type == "eia_api":
            found_items += site_eia_api_items(
                session,
                start_url,
                site_name,
                keywords,
        ...
        elif site_type == "fid_direct":
            # Direct download URLs (e.g., CAISO xlsx)
            found_items += site_fid_direct_items(
                start_urls,
                site_name,
                signal_type=site_cfg.get("signal_type", "fid_signal"),
                tier=int(site_cfg.get("tier", 1)),
            )
            break  # Process all URLs at once

        elif site_type == "fid_index":
            found_items += site_fid_index_items(
                session,
                start_url,
                site_name,
                keywords,
    ...
    # Save discovery cache for next run
    discovery_cache[site_name] = {"page_urls": [it.page_url for it in items]}

    # Filter out already-processed items BEFORE entering the loop
    seen_pages = state["seen_pages"]
    new_items = [it for it in items if it.page_url not in seen_pages]
    skipped = len(items) - len(new_items)

    print(f"Matched {len(items)} item(s) by title keywords: {keywords}")
    if skipped:
        print(f"  Skipping {skipped} already-processed item(s)")
    print(f"  Processing {len(new_items)} new item(s)")
    ...
    for it in new_items:
        ...
        # For IRENA: use button-based download directly (they block direct PDF links)
            ...
            state["seen_pages"].add(it.page_url)
            time.sleep(SLEEP)
            continue
        ...
        if not dl_links:
            ...
            try:
                ...
                if not export_links and not tables:
                    # No downloads, no tables — save the article text as .txt
                    try:
                        soup = BeautifulSoup(html, "html.parser")
                        ...
                        article_el = (
                            ...
                            soup.find("div", class_=re.compile(r"(post|article|entry|content|body)", re.I)) or
                            soup.find("main") or
                            soup.find("div", {"id": re.compile(r"(content|main|article)", re.I)})
                        )
                        body_text = (article_el or soup.body or soup).get_text(separator="\n", strip=True)
                        # Clean up excessive blank lines
                        body_text = re.sub(r"\n{3,}", "\n\n", body_text).strip()

                        if len(body_text) > 100:  # Only save if meaningful content
                            text_folder = os.path.join(out_dir, "text")
                            os.makedirs(text_folder, exist_ok=True)
                            fname = safe_filename(it.title) + ".txt"
                            fpath = os.path.join(text_folder, fname)
                            if not os.path.exists(fpath):
                                with open(fpath, "w", encoding="utf-8") as tf:
                                    tf.write(f"Title: {it.title}\n")
                                    tf.write(f"URL: {it.page_url}\n")
                                    tf.write(f"Date: {datetime.now().strftime('%Y-%m-%d')}\n")
                                    tf.write(f"{'='*70}\n\n")
                                    tf.write(body_text)
                                print(f"  [TEXT SAVED] {fname}")
                            else:
                                print(f"  [SKIP] Text already saved: {fname}")
                        else:
                            print("  (No data found on this page)")
                    except Exception as te:
                        print(f"  ! Error saving article text: {te}")
...
BUILTIN_SOURCES = [
    # ==== EIA RSS Feeds ====
    {"name": "EIA Today in Energy", "type": "rss_feed",
     "start_urls": ["https://www.eia.gov/rss/todayinenergy.xml"]},
    {"name": "EIA Press Releases", "type": "rss_feed",
     "start_urls": ["https://www.eia.gov/rss/press_rss.xml"]},
    {"name": "EIA Electricity", "type": "rss_feed",
     "start_urls": ["https://www.eia.gov/rss/electricity_rss.xml"]},
    {"name": "EIA Renewable", "type": "rss_feed",
     "start_urls": ["https://www.eia.gov/rss/renewable_rss.xml"]},
    {"name": "EIA Natural Gas", "type": "rss_feed",
     "start_urls": ["https://www.eia.gov/rss/ng_rss.xml"]},
    
    # ==== EIA Index Pages ====
    {"name": "EIA STEO", "type": "index_page",
     "start_urls": ["https://www.eia.gov/outlooks/steo/", "https://www.eia.gov/outlooks/steo/data.php"],
     "allowed_domains": ["eia.gov"]},
    {"name": "EIA AEO", "type": "index_page",
     "start_urls": ["https://www.eia.gov/outlooks/aeo/"],
     "allowed_domains": ["eia.gov"]},
    
    # ==== IEA ====
    {"name": "IEA News", "type": "rss_feed",
     "start_urls": ["https://www.iea.org/rss/news.xml"]},
    {"name": "IEA Hydrogen", "type": "index_page",
     "start_urls": ["https://www.iea.org/energy-system/low-emission-fuels/hydrogen",
                    "https://www.iea.org/reports?topic=hydrogen"],
     "allowed_domains": ["iea.org"]},
    
    # ==== IRENA ====
    {"name": "IRENA News", "type": "rss_feed",
     "start_urls": ["https://www.irena.org/rss/news"]},
    {"name": "IRENA Hydrogen", "type": "index_page",
     "start_urls": ["https://www.irena.org/Energy-Transition/Technology/Hydrogen"],
     "allowed_domains": ["irena.org"]},
    
    # ==== DOE/NREL ====
    {"name": "DOE News", "type": "rss_feed",
     "start_urls": ["https://www.energy.gov/rss/articles.xml"]},
    {"name": "DOE Hydrogen", "type": "index_page",
     "start_urls": ["https://www.energy.gov/eere/fuelcells/hydrogen-and-fuel-cell-technologies-office"],
     "allowed_domains": ["energy.gov"]},
    {"name": "NREL News", "type": "rss_feed",
     "start_urls": ["https://www.nrel.gov/news/rss.xml"]},
    {"name": "NREL Hydrogen", "type": "index_page",
     "start_urls": ["https://www.nrel.gov/hydrogen/"],
     "allowed_domains": ["nrel.gov"]},
    
    # ==== Hydrogen News ====
    {"name": "Hydrogen Insight", "type": "rss_feed",
     "start_urls": ["https://www.hydrogeninsight.com/rss"]},
    {"name": "H2 View", "type": "rss_feed",
     "start_urls": ["https://www.h2-view.com/feed/"]},
    {"name": "FuelCellsWorks", "type": "rss_feed",
     "start_urls": ["https://fuelcellsworks.com/feed/"]},
    {"name": "Recharge News", "type": "rss_feed",
     "start_urls": ["https://www.rechargenews.com/rss"]},
    
    # ==== Ammonia Energy ====
    {"name": "Ammonia Energy", "type": "rss_feed",
     "start_urls": ["https://ammoniaenergy.org/feed/"]},
    
    # ==== Hydrogen Council ====
    {"name": "Hydrogen Council", "type": "index_page",
     "start_urls": ["https://hydrogencouncil.com/en/reports/"],
     "allowed_domains": ["hydrogencouncil.com"]},
    
    # ==== World Bank ====
    {"name": "World Bank News", "type": "rss_feed",
     "start_urls": ["https://www.worldbank.org/en/news/rss.xml"]},
    {"name": "World Bank Energy", "type": "index_page",
     "start_urls": ["https://www.worldbank.org/en/topic/energy"],
     "allowed_domains": ["worldbank.org"]},
    
    # ==== Regional: Oman ====
    {"name": "HYDROM Oman", "type": "index_page",
     "start_urls": ["https://hydrom.om/", "https://hydrom.om/news/"],
     "allowed_domains": ["hydrom.om"]},
    
    # ==== Regional: Chile ====
    {"name": "Chile Energia", "type": "index_page",
     "start_urls": ["https://energia.gob.cl/hidrogeno-verde"],
     "allowed_domains": ["energia.gob.cl"]},
    
    # ==== Regional: EU ====
    {"name": "EU Hydrogen", "type": "index_page",
     "start_urls": ["https://energy.ec.europa.eu/topics/energy-systems-integration/hydrogen_en"],
     "allowed_domains": ["europa.eu", "ec.europa.eu"]},
    
    # ==== Lazard ====
    {"name": "Lazard LCOH", "type": "index_page",
     "start_urls": ["https://www.lazard.com/research-insights/"],
     "allowed_domains": ["lazard.com"]},
    
    # =========================================================================
    # FID SIGNALS - Interconnection Queues, Permits, Awards
    # =========================================================================
    
    # US Interconnection Queues (Tier 1-2 signals)
    {"name": "CAISO Queue", "type": "fid_direct",
     "start_urls": ["https://www.caiso.com/documents/publicqueuereport.xlsx"],
     "signal_type": "interconnection_queue", "tier": 1},
    
    {"name": "MISO GI Queue", "type": "fid_index",
     "start_urls": ["https://www.misoenergy.org/planning/resource-utilization/generator-interconnection/"],
     "allowed_domains": ["misoenergy.org"],
     "signal_type": "interconnection_queue", "tier": 1},
    
    {"name": "ERCOT GIS Report", "type": "fid_index",
     "start_urls": ["https://www.ercot.com/mp/data-products/data-product-details?id=pg7-200-er"],
     "allowed_domains": ["ercot.com"],
     "signal_type": "interconnection_milestones", "tier": 2},
    
    {"name": "PJM Queue", "type": "fid_index",
     "start_urls": ["https://www.pjm.com/planning/services-requests/interconnection-queues"],
     "allowed_domains": ["pjm.com"],
     "signal_type": "interconnection_queue", "tier": 1},
    
    {"name": "SPP GI Queue", "type": "fid_index",
     "start_urls": ["https://www.spp.org/engineering/generator-interconnection/"],
     "allowed_domains": ["spp.org"],
     "signal_type": "interconnection_queue", "tier": 1},
    
    {"name": "NYISO Queue", "type": "fid_index",
     "start_urls": ["https://www.nyiso.com/interconnections"],
     "allowed_domains": ["nyiso.com"],
     "signal_type": "interconnection_queue", "tier": 1},
    
    # FERC Filings (Tier 1-2 signals)
    {"name": "FERC eLibrary", "type": "fid_index",
     "start_urls": ["https://elibrary.ferc.gov/eLibrary/search"],
     "allowed_domains": ["ferc.gov"],
     "signal_type": "regulatory_filings", "tier": 1},
    
    # Permits (Tier 2 signals)
    {"name": "EPA ECHO", "type": "fid_index",
     "start_urls": ["https://echo.epa.gov/tools/web-services"],
     "allowed_domains": ["epa.gov"],
     "signal_type": "permits_api", "tier": 2},
    
    # Federal Awards (Tier 3 signals)
    {"name": "USAspending", "type": "fid_index",
     "start_urls": ["https://www.usaspending.gov/search"],
     "allowed_domains": ["usaspending.gov"],
     "signal_type": "federal_awards", "tier": 3},
    
    {"name": "DOE Loan Programs", "type": "fid_index",
     "start_urls": ["https://www.energy.gov/lpo/projects"],
     "allowed_domains": ["energy.gov"],
     "signal_type": "doe_loans", "tier": 3},
    
    # Global Queues (stubs - extend as needed)
    {"name": "UK NESO Connections", "type": "fid_index",
     "start_urls": ["https://www.neso.energy/industry-information/connections"],
     "allowed_domains": ["neso.energy"],
     "signal_type": "interconnection_queue", "tier": 1},
    
    {"name": "Australia AEMO", "type": "fid_index",
     "start_urls": ["https://aemo.com.au/energy-systems/electricity/national-electricity-market-nem/participate-in-the-market/network-connections"],
    ...
    # HYDROM - Official hydrogen project allocations
    {"name": "HYDROM Projects", "type": "fid_index",
     "start_urls": ["https://hydrom.om/", "https://hydrom.om/projects/", 
                    "https://hydrom.om/news/", "https://hydrom.om/blocks/"],
     "allowed_domains": ["hydrom.om"],
     "signal_type": "project_allocation", "tier": 2},
    
    # APSR - Authority for Public Services Regulation
    {"name": "Oman APSR", "type": "fid_index",
     "start_urls": ["https://apsr.om/en/licenses", "https://apsr.om/en/electricity",
                    "https://apsr.om/en/decisions", "https://apsr.om/ar/licenses"],
     "allowed_domains": ["apsr.om"],
     "signal_type": "generation_license", "tier": 2},
    
    # OPWP - Oman Power & Water Procurement
    {"name": "Oman OPWP", "type": "fid_index",
     "start_urls": ["https://omanpwp.om/new-projects/", "https://omanpwp.om/projects/",
                    "https://omanpwp.om/procurement/", "https://omanpwp.om/sector-news/"],
     "allowed_domains": ["omanpwp.om"],
     "signal_type": "ppa_award", "tier": 3},
    
    # OETC - Oman Electricity Transmission
    {"name": "Oman OETC", "type": "fid_index",
     "start_urls": ["https://oetc.om/", "https://oetc.om/en/grid", "https://oetc.om/en/projects"],
     "allowed_domains": ["oetc.om"],
     "signal_type": "grid_connection", "tier": 1},
    
    # OPAZ/Duqm SEZ
    {"name": "OPAZ Duqm SEZ", "type": "fid_index",
     "start_urls": ["https://opaz.gov.om/en", "https://opaz.gov.om/en/economic-zones/duqm",
                    "https://duqm.gov.om/", "https://duqm.gov.om/en/invest", "https://duqm.gov.om/en/news"],
     "allowed_domains": ["opaz.gov.om", "duqm.gov.om"],
     "signal_type": "sez_approval", "tier": 2},
    
    # MSX - Muscat Stock Exchange
    {"name": "Oman MSX", "type": "fid_index",
     "start_urls": ["https://www.msx.om/en/market/disclosures", "https://www.msx.om/en/news"],
     "allowed_domains": ["msx.om"],
     "signal_type": "company_disclosure", "tier": 3},
    
    # Tender Board Oman
    {"name": "Oman Tender Board", "type": "fid_index",
     "start_urls": ["https://etendering.tenderboard.gov.om/", "https://www.tenderboard.gov.om/"],
     "allowed_domains": ["tenderboard.gov.om"],
     "signal_type": "government_tender", "tier": 2},
    
    # OQ Company
    {"name": "OQ Oman", "type": "fid_index",
     "start_urls": ["https://www.oq.com/en/media/news", 
                    "https://www.oq.com/en/our-business/alternative-energy",
                    "https://www.oq.com/en/our-business/alternative-energy/hydrogen"],
     "allowed_domains": ["oq.com"],
     "signal_type": "company_announcement", "tier": 2},
    
    # Ministry of Energy and Minerals
    {"name": "Oman MEM", "type": "fid_index",
     "start_urls": ["https://mem.gov.om/en/", "https://mem.gov.om/en/news", "https://mem.gov.om/ar/"],
     "allowed_domains": ["mem.gov.om"],
     "signal_type": "policy_announcement", "tier": 2},
    
    # Oman News Agency
    {"name": "Oman News Agency", "type": "fid_index",
     "start_urls": ["https://omannews.gov.om/topics/en/15/10", "https://omannews.gov.om/topics/ar/15/10"],
     "allowed_domains": ["omannews.gov.om"],
     "signal_type": "news", "tier": 1},
    ...
    {"name": "Chile CEN", "type": "fid_index",
     "start_urls": ["https://www.coordinador.cl/operacion/",
                    "https://www.coordinador.cl/desarrollo/conexiones/",
                    "https://www.coordinador.cl/informes-y-estadisticas/"],
     "allowed_domains": ["coordinador.cl"],
     "signal_type": "grid_connection_queue", "tier": 1},
    
    # CNE - Comisión Nacional de Energía
    {"name": "Chile CNE", "type": "fid_index",
     "start_urls": ["https://www.cne.cl/estadisticas/electricidad/",
                    "https://www.cne.cl/normativas/electricidad/",
                    "https://www.cne.cl/noticias/"],
     "allowed_domains": ["cne.cl"],
     "signal_type": "energy_permit", "tier": 2},
    
    # SEA - Environmental permits
    {"name": "Chile SEA", "type": "fid_index",
     "start_urls": ["https://www.sea.gob.cl/", "https://www.sea.gob.cl/noticias"],
     "allowed_domains": ["sea.gob.cl"],
     "signal_type": "environmental_permit", "tier": 2},
    
    # Ministerio de Energía
    {"name": "Chile Min Energia", "type": "fid_index",
     "start_urls": ["https://energia.gob.cl/", "https://energia.gob.cl/hidrogeno-verde",
                    "https://energia.gob.cl/noticias"],
     "allowed_domains": ["energia.gob.cl"],
     "signal_type": "h2_project_registry", "tier": 2},
    
    # CORFO
    {"name": "Chile CORFO", "type": "fid_index",
     "start_urls": ["https://www.corfo.cl/sites/cpp/hidrogeno-verde",
                    "https://www.corfo.cl/sites/cpp/convocatorias"],
     "allowed_domains": ["corfo.cl"],
     "signal_type": "h2_grant", "tier": 3},
    
    # SEC - Operating permits
    {"name": "Chile SEC", "type": "fid_index",
     "start_urls": ["https://www.sec.cl/", "https://www.sec.cl/area-electrica/"],
     "allowed_domains": ["sec.cl"],
     "signal_type": "operating_permit", "tier": 2},
    
    # Chile Compra
    {"name": "Chile Compra", "type": "fid_index",
     "start_urls": ["https://www.mercadopublico.cl/Home"],
     "allowed_domains": ["mercadopublico.cl"],
     "signal_type": "government_procurement", "tier": 3},
    
    # CMF - Company disclosures
    {"name": "Chile CMF", "type": "fid_index",
     "start_urls": ["https://www.cmfchile.cl/portal/principal/613/w3-propertyvalue-43419.html"],
     "allowed_domains": ["cmfchile.cl"],
     "signal_type": "company_disclosure", "tier": 3},
    
    # H2 Chile
    {"name": "H2 Chile", "type": "fid_index",
     "start_urls": ["https://h2chile.cl/", "https://h2chile.cl/noticias/", "https://h2chile.cl/proyectos/"],
     "allowed_domains": ["h2chile.cl"],
     "signal_type": "industry_news", "tier": 1},
    
    # Generadoras de Chile
    {"name": "Generadoras Chile", "type": "fid_index",
     "start_urls": ["https://generadoras.cl/", "https://generadoras.cl/prensa"],
     "allowed_domains": ["generadoras.cl"],
     "signal_type": "industry_news", "tier": 1},
    
    # InvestChile
    {"name": "InvestChile", "type": "fid_index",
     "start_urls": ["https://investchile.gob.cl/", "https://investchile.gob.cl/sectores/energia/"],
     "allowed_domains": ["investchile.gob.cl"],
     "signal_type": "investment_promotion", "tier": 1},
]

# Multilingual keywords for FID discovery
FID_KEYWORDS_ARABIC = [
    "هيدروجين", "الهيدروجين الأخضر", "أمونيا", "طاقة متجددة", "طاقة شمسية",
    "طاقة الرياح", "رخصة", "تصريح", "موافقة", "مناقصة", "عقد", "منحة",
    "اتفاقية", "مشروع", "استثمار", "كهرباء", "شبكة", "توليد", "محطة",
    "دقم", "صحار", "صلالة",
]

FID_KEYWORDS_SPANISH = [
    "hidrógeno", "hidrógeno verde", "electrolizador", "amoníaco", "amoníaco verde",
    "energía renovable", "energía solar", "energía eólica", "permiso", "licencia",
    "aprobación", "autorización", "licitación", "contrato", "adjudicación",
    "subsidio", "convenio", "acuerdo", "proyecto", "inversión", "conexión",
    "transmisión", "generación", "central", "Magallanes", "Antofagasta", "Atacama",
    "evaluación ambiental", "estudio de impacto",
]


def get_default_config():
    """Return default configuration with built-in sources."""
    return {
        "keywords": [
            # English
            "hydrogen", "electrolyzer", "LCOH", "green hydrogen", "ammonia",
            "renewable", "wind", "solar", "electricity price", "PPA",
            "energy outlook", "energy transition",
            "interconnection", "queue", "permit", "license", "tender",
            "award", "grant", "FID", "final investment", "EPC", "offtake",
            # Arabic (Oman)
            "هيدروجين", "الهيدروجين الأخضر", "أمونيا", "طاقة متجددة",
            "رخصة", "تصريح", "مناقصة", "عقد", "مشروع", "دقم",
            # Spanish (Chile)
            "hidrógeno", "hidrógeno verde", "electrolizador", "amoníaco",
            "energía renovable", "permiso", "licencia", "licitación",
            "proyecto", "Magallanes", "Atacama",
        ],
        "output_dir": "downloads",
        "sites": BUILTIN_SOURCES,
    }
...
def main() -> None:
    ...
    parser.add_argument("--use-builtin", action="store_true",
                        help="Use built-in sources (EIA, IEA, IRENA, etc.) without sites.yaml")
    parser.add_argument("--create-config", action="store_true",
                        help="Create api_keys.json template for API keys")
    parser.add_argument("--check-api-keys", action="store_true",
                        help="Check API key configuration status")
    ...
    if args.create_config:
        ...
        if config_path.exists():
            ...
            template = {
                ...
                "_comment": "Get free API keys from:",
    ...
    if args.check_api_keys:
        ...
        print("-" * 40)
        print("\nTo set up API keys:")
        print("  python website_dl.py --create-config")
        print("  Then edit api_keys.json")
        return
    
    yaml_path = args.yaml
    
    # Load config - use built-in if requested or if no YAML exists
    if args.use_builtin or not os.path.exists(yaml_path):
        if not os.path.exists(yaml_path):
            print(f"[INFO] No {yaml_path} found, using built-in sources")
    ...
    for key in ("seen_pages", "seen_download_urls", "seen_hashes"):
        state[key] = set(state.get(key, []))

    sites = cfg.get("sites", [])
    print(f"\nProcessing {len(sites)} source(s)...")
    print(f"  ({len(state['seen_pages'])} pages already processed from previous runs)\n")
