...
# Knowledge Base Integration (for trained document context)
import chromadb
...
def init_knowledge_base(db_path: str = './knowledge_base_db'):
    """Initialize the knowledge base for context-aware analysis."""
    global KNOWLEDGE_BASE, KB_EMBEDDER

    try:
        print("Loading knowledge base...")
        client = chromadb.PersistentClient(path=db_path)
        KNOWLEDGE_BASE = client.get_or_create_collection(name="tea_knowledge")

        doc_count = KNOWLEDGE_BASE.count()
        if doc_count == 0:
            print("⚠️ Knowledge base is empty. Run llm_training_system.py steps 1 & 2 first.")
            KNOWLEDGE_BASE = None
            return False
        ...
        KB_EMBEDDER = SentenceTransformer('all-MiniLM-L6-v2')
        print("✓ Embedding model loaded")

        return True
    except Exception as e:
        print(f"⚠️ Could not load knowledge base: {e}")
        print("  Analysis will proceed without trained document context.")
        KNOWLEDGE_BASE = None
        KB_EMBEDDER = None
        return False
...
def search_knowledge_base(query: str, top_k: int = 5) -> list:
    ...
    Args:
        query: Search query (e.g., article title + snippet)
    ...
    if KNOWLEDGE_BASE is None or KB_EMBEDDER is None:
        return []

    try:
        # Generate query embedding
        query_embedding = KB_EMBEDDER.encode([query]).tolist()
...
def get_kb_context_summary(article_title: str, article_content: str = '', top_k: int = 3) -> str:
    """
    Get a summary of relevant knowledge base context for an article.

    Returns a formatted string with relevant excerpts from training documents.
    """
    query = f"{article_title} {article_content[:500] if article_content else ''}"
    context_docs = search_knowledge_base(query, top_k=top_k)

    if not context_docs:
        return ""

    summary_parts = ["📚 Relevant Context from Training Documents:"]
    for i, doc in enumerate(context_docs, 1):
        relevance_pct = doc['relevance'] * 100
        excerpt = doc['text'][:300] + "..." if len(doc['text']) > 300 else doc['text']
        summary_parts.append(f"  [{i}] ({relevance_pct:.0f}% relevant) {doc['source']}")
        summary_parts.append(f"      {excerpt}")

    return "\n".join(summary_parts)

def analyze_impact_from_kb(article_title: str, article_content: str, baseline: dict, region: str) -> dict:
    """
    Use the knowledge base to intelligently determine NPV/LCOH impact.

    Analyzes training documents to find:
    1. Similar historical events and their impacts
    2. Price/cost trends mentioned in relevant documents
    3. Market patterns that correlate with the news
    ...
    query = f"{article_title} {article_content[:1000] if article_content else ''}"
    kb_docs = search_knowledge_base(query, top_k=5)

    if not kb_docs:
        result['reasoning'] = 'No relevant training documents found'
        return result

    # Analyze the retrieved documents for impact indicators
    combined_text = ' '.join([doc['text'].lower() for doc in kb_docs])
    avg_relevance = sum(doc['relevance'] for doc in kb_docs) / len(kb_docs)

    # Impact patterns to look for in training documents
    impact_signals = {
        'positive_npv': [
            ('cost reduction', 0.03), ('price decrease', 0.02), ('efficiency improvement', 0.02),
            ('subsidy', 0.04), ('incentive', 0.03), ('tax credit', 0.05),
            ('demand increase', 0.03), ('market growth', 0.02), ('expansion', 0.02),
            ('competitor exit', 0.04), ('project cancelled', 0.03),
            ('breakthrough', 0.03), ('innovation', 0.02), ('record low', 0.03)
        ],
        'negative_npv': [
            ('cost increase', -0.03), ('price increase', -0.02), ('tariff', -0.02),
            ('delay', -0.02), ('postpone', -0.02), ('uncertainty', -0.01),
            ('oversupply', -0.04), ('competition', -0.02), ('new entrant', -0.02),
            ('regulation', -0.01), ('restriction', -0.02), ('challenge', -0.01),
            ('decline', -0.02), ('downturn', -0.02), ('risk', -0.01)
        ],
        'positive_lcoh': [  # Positive = cost reduction (lower LCOH is good)
            ('electrolyzer cost', -0.10), ('renewable cost', -0.05), ('efficiency', -0.05),
            ('learning curve', -0.03), ('scale up', -0.04), ('mass production', -0.05)
        ],
        'negative_lcoh': [  # Negative = cost increase (higher LCOH is bad)
            ('electricity price', 0.08), ('energy cost', 0.06), ('supply chain', 0.04),
            ('material cost', 0.05), ('labor cost', 0.03)
        ]
    }

    # Calculate impact based on patterns found in KB documents
    npv_impact = 0
    lcoh_impact = 0
    reasons = []
    sources = []
    ...
    article_lower = (article_title + ' ' + (article_content or '')).lower()
    ...
    for doc in kb_docs:
        percentages = re.findall(percentage_pattern, doc['text'])
        if percentages and avg_relevance > 0.3:
            # If KB docs mention specific percentages, consider them
            for pct in percentages[:2]:  # Take first 2 percentages found
                pct_val = float(pct)
                if 1 <= pct_val <= 30:  # Reasonable impact range
                    # Adjust impact based on KB percentage mentions
                    if any(word in article_lower for word in ['increase', 'rise', 'growth']):
                        if any(word in article_lower for word in ['cost', 'price', 'expense']):
                            npv_impact -= baseline['npv'] * (pct_val / 100) * 0.3 * avg_relevance
                        else:
                            npv_impact += baseline['npv'] * (pct_val / 100) * 0.3 * avg_relevance
                    elif any(word in article_lower for word in ['decrease', 'fall', 'drop', 'reduction']):
                        if any(word in article_lower for word in ['cost', 'price', 'expense']):
                            npv_impact += baseline['npv'] * (pct_val / 100) * 0.3 * avg_relevance
                        else:
                            npv_impact -= baseline['npv'] * (pct_val / 100) * 0.3 * avg_relevance
...
def analyze_cumulative_impact():
    ...
    for i, article in enumerate(relevant_articles, 1):
        ...
        impact_type = 'neutral'
        event_description = ''
        impact_reasoning = ''  # Explains HOW the impact was calculated
        actions = []
        ...
        if KNOWLEDGE_BASE is not None:
            kb_docs = search_knowledge_base(f"{title} {content[:500] if content else ''}", top_k=3)
            if kb_docs:
                kb_parts = []
                for d in kb_docs:
                    source_name = d.get('source', 'unknown')
                    source_type = d.get('source_type', '')
                    # L2 distance: lower = more similar. Convert to intuitive label
                    raw_relevance = d.get('relevance', 0)  # 1 - L2_distance
                    if raw_relevance > 0.5:
                        rel_label = "HIGH"
                    elif raw_relevance > 0:
                        rel_label = "MODERATE"
                    else:
                        rel_label = "LOW"
                    # Show source name and relevance, with a short excerpt
                    clean_text = ' '.join(d['text'].split()[:25])  # First 25 words, cleaned
        ...
        if any(word in article_lower for word in ['cancel', 'withdraw', 'exit', 'abandon', 'shelved']):
            if is_region_specific:
                ...
                if deep_dive['found_details']:
                    ...
                    impact_reasoning = f"Rule: Competitor cancellation in {your_region} | Formula: baseline NPV ${baseline['npv']:.1f}M x 4% = ${npv_change:+.1f}M | Deep dive confirmed reasons"
                    ...
                    if any(word in reasons_text for word in ['financing', 'offtake', 'demand', 'uneconomic']):
                        ...
                        impact_reasoning = f"Rule: Competitor cancellation in {your_region} (market concerns) | Formula: baseline NPV ${baseline['npv']:.1f}M x 3% = ${npv_change:+.1f}M | Reduced from 4% due to market-level concerns in deep dive"
                    ...
                    impact_reasoning = f"Rule: Competitor cancellation (unknown reason) | Formula: probability-weighted expected value across 4 scenarios = ${npv_change:+.1f}M | No clear reasons found - used scenario analysis"
                ...
                impact_reasoning = f"Rule: Competitor cancellation (global/other region) | Formula: baseline NPV ${baseline['npv']:.1f}M x 2% = ${npv_change:+.1f}M | Reduced weight: not in your region"
        ...
        # Competitor FID/offtake - STRONGER negative if in same region
            ...
            if is_region_specific:
                ...
                impact_reasoning = f"Rule: Competitor FID in same region | Formula: baseline NPV ${baseline['npv']:.1f}M x -3% = ${npv_change:+.1f}M | Direct competition: competitor advancing in {your_region}"
                ...
                impact_reasoning = f"Rule: Competitor FID (global/other region) | Formula: baseline NPV ${baseline['npv']:.1f}M x -1% = ${npv_change:+.1f}M | Indirect: general market competition pressure"
        ...
        # Policy changes - 45V affects all hydrogen projects (global market signal)
        elif '45v' in article_lower or '45V' in title:
            us_multiplier = 1.0 if your_region == 'Houston' else 0.3  # Stronger for US, but affects global H2 market
            if 'extended' in article_lower or 'extension' in article_lower or 'reprieve' in article_lower:
                npv_change = baseline['npv'] * 0.15 * us_multiplier
                impact_type = 'positive'
                event_description = f"45V tax credit extended (global H2 market signal)"
                impact_reasoning = f"Rule: 45V extension | Formula: baseline NPV ${baseline['npv']:.1f}M x 15% x {us_multiplier} (region weight) = ${npv_change:+.1f}M | 45V $3/kg credit boosts US H2 supply, affecting global trade"
                actions.append("Monitor impact on US hydrogen supply competing with your project")
            elif 'repeal' in article_lower or 'end' in article_lower or 'drop' in article_lower or 'cut' in article_lower:
                npv_change = baseline['npv'] * 0.05 * us_multiplier  # Less US competition = good for non-US
                if your_region != 'Houston':
                    impact_type = 'positive'
                    event_description = "45V repeal reduces US hydrogen competition"
                    impact_reasoning = f"Rule: 45V repeal (non-US project) | Formula: baseline NPV ${baseline['npv']:.1f}M x 5% x {us_multiplier} = ${npv_change:+.1f}M | Less US competition benefits non-US exporters"
                    actions.append("US projects delayed - opportunity for non-US suppliers")
                else:
                    npv_change = -baseline['npv'] * 0.25
                    impact_type = 'negative'
                    event_description = "45V repeal threatens Houston project economics"
                    impact_reasoning = f"Rule: 45V repeal (Houston project) | Formula: baseline NPV ${baseline['npv']:.1f}M x -25% = ${npv_change:+.1f}M | Direct loss of $3/kg production tax credit"
                    actions.append("CRITICAL: Re-evaluate Houston project viability")
            elif 'deadline' in article_lower or 'requirement' in article_lower:
                npv_change = -baseline['npv'] * 0.03 * us_multiplier
                impact_type = 'negative'
                event_description = f"45V policy uncertainty (global market impact)"
                impact_reasoning = f"Rule: 45V policy uncertainty | Formula: baseline NPV ${baseline['npv']:.1f}M x -3% x {us_multiplier} = ${npv_change:+.1f}M | Regulatory uncertainty creates investment hesitancy"
                actions.append("Monitor 45V developments for competitive implications")
        ...
        # Infrastructure development - MUCH stronger if in same region
            ...
            if is_region_specific:
                ...
                impact_reasoning = f"Rule: Regional infrastructure | Formula: NPV ${baseline['npv']:.1f}M x 8% = ${npv_change:+.1f}M, LCOH -$0.15/kg | Port/infra in {your_region} reduces logistics costs and enables export"
                ...
                impact_reasoning = f"Rule: Global infrastructure | Formula: NPV ${baseline['npv']:.1f}M x 2% = ${npv_change:+.1f}M, LCOH -$0.03/kg | Global best practices / learning spillover"
        ...
        # Electrolyzer costs - global impact (same for all regions)
            ...
            if 'fall' in article_lower or 'drop' in article_lower or 'low' in article_lower:
                ...
                impact_reasoning = f"Rule: Electrolyzer cost decline | Formula: NPV ${baseline['npv']:.1f}M x 5% = ${npv_change:+.1f}M, LCOH -$0.18/kg | Electrolyzer CAPEX is ~40% of LCOH; cost drops directly improve project economics"
        ...
        # Oversupply warnings - global impact
            ...
            impact_reasoning = f"Rule: Market oversupply | Formula: NPV ${baseline['npv']:.1f}M x -8% = ${npv_change:+.1f}M | Oversupply depresses hydrogen prices, reduces revenue projections"
        ...
        # PPA price changes - stronger if region-specific
            ...
            if 'increase' in article_lower or 'rise' in article_lower:
                ...
                impact_reasoning = f"Rule: PPA price increase | Formula: NPV ${baseline['npv']:.1f}M x -3% x {multiplier} (scope) = ${npv_change:+.1f}M, LCOH +${lcoh_change:.2f}/kg | Electricity ~60% of LCOH; PPA increases directly raise production cost"
            elif 'decrease' in article_lower or 'fall' in article_lower:
                npv_change = baseline['npv'] * 0.03 * multiplier
                lcoh_change = -0.08 * multiplier
                impact_type = 'positive'
                event_description = f"PPA costs decreasing ({impact_scope})"
                impact_reasoning = f"Rule: PPA price decrease | Formula: NPV ${baseline['npv']:.1f}M x 3% x {multiplier} (scope) = ${npv_change:+.1f}M, LCOH ${lcoh_change:+.2f}/kg | Lower electricity costs reduce LCOH"
        
        # Broader keyword matching for articles not caught by specific patterns above
        if npv_change == 0 and lcoh_change == 0:
            scope_mult = 1.0 if is_region_specific else 0.3

            # Subsidy/incentive/tax credit news (not 45V specific)
            if any(w in article_lower for w in ['subsidy', 'incentive', 'tax credit', 'grant', 'funding']):
                if any(w in article_lower for w in ['approved', 'launch', 'new', 'propose', 'pass']):
                    npv_change = baseline['npv'] * 0.03 * scope_mult
                    impact_type = 'positive'
                    event_description = f"Policy support for hydrogen ({impact_scope})"
                    impact_reasoning = f"Rule: Subsidy/incentive positive | Formula: NPV ${baseline['npv']:.1f}M x 3% x {scope_mult} (scope) = ${npv_change:+.1f}M | New policy support improves project revenue or reduces costs"
                    actions.append("Review eligibility for announced incentives")
                elif any(w in article_lower for w in ['cut', 'end', 'repeal', 'reduce']):
                    npv_change = -baseline['npv'] * 0.03 * scope_mult
                    impact_type = 'negative'
                    event_description = f"Subsidy/incentive reduction ({impact_scope})"
                    impact_reasoning = f"Rule: Subsidy/incentive negative | Formula: NPV ${baseline['npv']:.1f}M x -3% x {scope_mult} (scope) = ${npv_change:+.1f}M | Reduced support worsens project economics"
                    actions.append("Assess impact on project economics")

            # Technology breakthrough / efficiency improvements
            elif any(w in article_lower for w in ['breakthrough', 'record', 'efficiency', 'innovation']):
                if any(w in article_lower for w in ['electroly', 'hydrogen', 'renewable', 'solar', 'wind']):
                    npv_change = baseline['npv'] * 0.02 * scope_mult
                    lcoh_change = -0.05 * scope_mult
                    impact_type = 'positive'
                    event_description = f"Technology advancement ({impact_scope})"
                    impact_reasoning = f"Rule: Technology breakthrough | Formula: NPV ${baseline['npv']:.1f}M x 2% x {scope_mult} = ${npv_change:+.1f}M, LCOH ${lcoh_change:+.2f}/kg | Technology improvements reduce future production costs"
                    actions.append("Evaluate technology for project applicability")

            # Demand growth / market expansion
            elif any(w in article_lower for w in ['demand', 'market growth', 'expansion', 'ramp up']):
                if any(w in article_lower for w in ['hydrogen', 'ammonia', 'green', 'clean']):
                    npv_change = baseline['npv'] * 0.02 * scope_mult
                    impact_type = 'positive'
                    event_description = f"Hydrogen demand signal ({impact_scope})"
                    impact_reasoning = f"Rule: Demand growth | Formula: NPV ${baseline['npv']:.1f}M x 2% x {scope_mult} = ${npv_change:+.1f}M | Growing demand supports pricing power and offtake likelihood"
            ...
            # Delay / postpone / challenges
                ...
                if is_region_specific:
                    npv_change = -baseline['npv'] * 0.02
                    impact_type = 'negative'
                    event_description = f"Project delays in {your_region}"
                    impact_reasoning = f"Rule: Regional delay | Formula: NPV ${baseline['npv']:.1f}M x -2% = ${npv_change:+.1f}M | Delays in {your_region} may signal regulatory/infrastructure challenges affecting your project"
                    actions.append("Assess if delays affect your project timeline")
                elif any(w in article_lower for w in ['hydrogen', 'green', 'electroly']):
                    npv_change = -baseline['npv'] * 0.01
                    impact_type = 'negative'
                    event_description = "Industry delays (global signal)"
                    impact_reasoning = f"Rule: Global industry delay | Formula: NPV ${baseline['npv']:.1f}M x -1% = ${npv_change:+.1f}M | Industry-wide delays may indicate systemic challenges"

            # Partnership / agreement / deal
            elif any(w in article_lower for w in ['partnership', 'agreement', 'deal', 'contract', 'mou']):
                if is_region_specific:
                    npv_change = baseline['npv'] * 0.02
                    impact_type = 'positive'
                    event_description = f"Market activity in {your_region}"
                    impact_reasoning = f"Rule: Regional partnership/deal | Formula: NPV ${baseline['npv']:.1f}M x 2% = ${npv_change:+.1f}M | Deal activity in {your_region} signals market maturity and offtake potential"
                    actions.append(f"Explore similar partnerships in {your_region}")
                elif any(w in article_lower for w in ['hydrogen', 'ammonia', 'green', 'electroly']):
                    npv_change = baseline['npv'] * 0.005
                    impact_type = 'positive'
                    event_description = "Hydrogen deal activity (global)"
                    impact_reasoning = f"Rule: Global deal activity | Formula: NPV ${baseline['npv']:.1f}M x 0.5% = ${npv_change:+.1f}M | Global deal flow supports overall market confidence"

            # Electrolyzer news (broader than existing - catches deliveries, orders, installations)
            elif 'electroly' in article_lower:
                if any(w in article_lower for w in ['install', 'deliver', 'complete', 'order', 'commission']):
                    npv_change = baseline['npv'] * 0.01
                    lcoh_change = -0.03
                    impact_type = 'positive'
                    event_description = "Electrolyzer deployment progress (global)"
                    impact_reasoning = f"Rule: Electrolyzer deployment | Formula: NPV ${baseline['npv']:.1f}M x 1% = ${npv_change:+.1f}M, LCOH -$0.03/kg | Deployment scale-up drives learning curves and cost reduction"
                elif any(w in article_lower for w in ['pause', 'cancel', 'dip', 'struggle']):
                    npv_change = -baseline['npv'] * 0.01
                    impact_type = 'negative'
                    event_description = "Electrolyzer market challenges (global)"
                    impact_reasoning = f"Rule: Electrolyzer challenges | Formula: NPV ${baseline['npv']:.1f}M x -1% = ${npv_change:+.1f}M | Supply chain issues may delay equipment delivery or raise costs"
    ...
    if KNOWLEDGE_BASE is not None:
        ...
        print(f"📚 Knowledge base: NOT LOADED (run llm_training_system.py steps 1 & 2)")
    print(f"{'='*70}\n")
    # Display results
    print(f"\n{'='*70}")
    ...
    print(f"Baseline Data (from SHARE TEA Model):")
    print(f"   Region:            {your_region}")
    print(f"   NPV:               ${baseline['npv']:.1f}M")
    print(f"   LCOH:              ${baseline['lcoh']:.2f}/kg")
    if baseline.get('lcoa'):
        print(f"   LCOA:              ${baseline['lcoa']:.2f}/kg")
    print(f"   Timeline:          {baseline['timeline']} months")
    if baseline.get('capex'):
        print(f"   CAPEX:             ${baseline['capex']:.1f}M")
    if baseline.get('irr'):
        print(f"   IRR:               {baseline['irr']:.2f}%")
    print(f"   Source:            {baseline.get('source', {}).get('model', 'SHARE')} model output")
    print()

    print(f"Impact Assessment Rules:")
    print(f"   Each article is classified by keyword matching into event categories.")
    print(f"   NPV impact = Baseline NPV x Category Weight x Scope Multiplier")
    print(f"   • Region-specific articles: scope multiplier = 1.0 (direct impact)")
    print(f"   • Global articles: scope multiplier = 0.3 (indirect/spillover)")
    print()

    print(f"   Category Weights Used:")
    print(f"   {'Category':<35} {'NPV %':>8} {'LCOH':>10}")
    print(f"   {'─'*55}")
    print(f"   {'Competitor cancellation (region)':<35} {'±4%':>8} {'—':>10}")
    print(f"   {'Competitor cancellation (global)':<35} {'±2%':>8} {'—':>10}")
    print(f"   {'Competitor FID (region)':<35} {'-3%':>8} {'—':>10}")
    print(f"   {'Competitor FID (global)':<35} {'-1%':>8} {'—':>10}")
    print(f"   {'Infrastructure (region)':<35} {'+8%':>8} {'-$0.15/kg':>10}")
    print(f"   {'Infrastructure (global)':<35} {'+2%':>8} {'-$0.03/kg':>10}")
    print(f"   {'Electrolyzer cost decline':<35} {'+5%':>8} {'-$0.18/kg':>10}")
    print(f"   {'Market oversupply':<35} {'-8%':>8} {'—':>10}")
    print(f"   {'PPA price change':<35} {'±3%':>8} {'±$0.08/kg':>10}")
    print(f"   {'45V extension (US=1.0, other=0.3)':<35} {'+15%':>8} {'—':>10}")
    print(f"   {'45V repeal (Houston)':<35} {'-25%':>8} {'—':>10}")
    print(f"   {'Subsidy/incentive':<35} {'±3%':>8} {'—':>10}")
    print(f"   {'Technology breakthrough':<35} {'+2%':>8} {'-$0.05/kg':>10}")
    print(f"   {'Demand growth':<35} {'+2%':>8} {'—':>10}")
    print(f"   {'Partnership/deal (region)':<35} {'+2%':>8} {'—':>10}")
    print(f"   {'Delay (region)':<35} {'-2%':>8} {'—':>10}")
    print()

    kb_doc_count = KNOWLEDGE_BASE.count() if KNOWLEDGE_BASE is not None else 0
    print(f"Knowledge Base:")
    if kb_doc_count > 0:
        print(f"   Status:            ACTIVE ({kb_doc_count:,} training documents)")
        print(f"   Role:              Fallback for articles not matching keyword rules")
        print(f"   Method:            Semantic search (all-MiniLM-L6-v2 embeddings)")
        print(f"   Confidence threshold: >20% to assign impact")
    else:
    ...
    print(f"   Articles analyzed: {len(relevant_articles)}")
    print(f"   With financial impact: {articles_with_impact} ({articles_with_impact/len(relevant_articles)*100:.0f}%)")
    print(f"   No impact (unmatched): {len(relevant_articles) - articles_with_impact} ({(len(relevant_articles)-articles_with_impact)/len(relevant_articles)*100:.0f}%)")
    ...
    <div class="{'success' if cumulative_npv_change > 0 else 'critical' if cumulative_npv_change < 0 else 'neutral'}">
        ...
        <p style="font-size: 1.5em; margin: 15px 0;">
            <strong>NPV Change: ${cumulative_npv_change:+.1f}M ({cumulative_npv_change/baseline['npv']*100:+.1f}%)</strong>
        </p>
        <p style="font-size: 1.2em;">
            <strong>Updated NPV: ${(baseline['npv'] + cumulative_npv_change):.1f}M</strong>
        </p>
        ...
        <div class="section">
            <table>
                ...
                <tr class="positive">
                    ...
                    <td>${sum([e['npv_change'] for e in positive_events]):+.1f}M</td>
                </tr>
                <tr class="negative">
                    <td>⚠️ Negative Events</td>
                    <td>{len(negative_events)}</td>
                    <td>${sum([e['npv_change'] for e in negative_events]):+.1f}M</td>
                </tr>
                <tr class="neutral">
                    <td>🟡 Neutral/Mixed Events</td>
                    <td>{len(neutral_events)}</td>
                    <td>${sum([e['npv_change'] for e in neutral_events]):+.1f}M</td>
                </tr>
            </table>
        </div>
        """
...
def display_uncertainty_scenarios(analysis, baseline, your_region):
    ...
    for scenario in analysis['scenarios']:
        ...
        print(f"📈 NPV Change: ${scenario['npv_change']:+.1f}M ({scenario['npv_change']/baseline['npv']*100:+.1f}%)")
        if scenario['timeline_change'] != 0:
            print(f"⚡ Timeline: {scenario['timeline_change']:+d} months")
        print(f"💰 Updated NPV: ${(baseline['npv'] + scenario['npv_change']):.1f}M\n")
        
        # Show breakdown if available
        if 'npv_components' in scenario:
            print(f"Breakdown:")
            for component, value in scenario['npv_components'].items():
                print(f"  {component.replace('_', ' ').title()}: ${value:+.1f}M")
            print()
    ...
    for scenario in analysis['scenarios']:
        ...
        print(f"Scenario {scenario['number']} ({scenario['name'][:20]}...): "
              f"{scenario['probability']*100:>2.0f}% × ${scenario['npv_change']:+.1f}M = ${contrib:+.1f}M")
    
    print(f"{'═'*70}")
    print(f"Expected NPV Impact: ${analysis['expected_value']:+.1f}M\n")
    print(f"Confidence Interval: ${analysis['confidence_interval']['worst_case']:+.1f}M "
          f"to ${analysis['confidence_interval']['best_case']:+.1f}M (wide range!)\n")
    ...
    print(f"   → Updated NPV: ${(baseline['npv'] + analysis['scenarios'][0]['npv_change']):.1f}M ✅\n")
    ...
    print(f"   → Updated NPV: ${(baseline['npv'] + analysis['scenarios'][1]['npv_change']):.1f}M ⚠️\n")
    ...
    print(f"   → Updated NPV: ${(baseline['npv'] + analysis['scenarios'][2]['npv_change']):.1f}M 🚨\n")
    
    print(f"IF Scenario 4 confirmed (Financing):")
    print(f"   → ADAPT: Adjust financing strategy, budget higher costs")
    print(f"   → Updated NPV: ${(baseline['npv'] + analysis['scenarios'][3]['npv_change']):.1f}M ⚠️\n")
...
def get_region_from_context_or_prompt(feature_name="this feature"):
    ...
    if SESSION_CONTEXT['analysis_run']:
        ...
        print(f"   Articles: {SESSION_CONTEXT['articles_analyzed']}, "
              f"NPV Impact: ${SESSION_CONTEXT['cumulative_npv_change']:+.1f}M\n")
...
def view_deal_flow():
    ...
    for deal in deals:
        ...
        value_str = f"${value:.1f}M" if value else "N/A"
...
def main():
    ...
    # Load knowledge base (trained documents)
